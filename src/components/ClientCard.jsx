...
import { Edit2, Trash2, Phone, Mail, Clock, AlertCircle, MessageSquare } from 'lucide-react'
    ...
    >
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '8px' }}>
        ...
        <div style={{ display: 'flex', gap: '5px', alignItems: 'center' }}>
          ...
          <Btn icon={Edit2} onClick={onEdit} />
          <Btn icon={Trash2} onClick={onDelete} danger />
        </div>
      </div>
      ...
      {!logOpen ? (
        ...
        ><MessageSquare size={10} /> LOG CONTACT</button>
      ) : (
...
function Btn({ icon: Icon, onClick, danger }) {
  return (
    <button onClick={onClick} style={{ background: 'transparent', border: '1px solid var(--border)', borderRadius: '4px', padding: '4px 6px', cursor: 'pointer', color: danger ? 'var(--danger)' : 'var(--muted)', display: 'flex', alignItems: 'center', transition: 'all 0.15s' }}
