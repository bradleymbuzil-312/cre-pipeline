    ...
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '24px' }}>
        ...
        <div style={{ display: 'flex', gap: '2px', background: 'var(--surface2)', borderRadius: '6px', padding: '2px', border: '1px solid var(--border)' }}>
          {[['pipeline', 'Deals'], ['clients', 'Clients']].map(([v, label]) => (
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
        <button
          ...
          title={alertCount > 0 ? `${alertCount} follow-up${alertCount > 1 ? 's' : ''} due` : 'No follow-ups due'}
          style={{
            position: 'relative', background: 'transparent',
            border: '1px solid var(--border)', borderRadius: '6px',
            padding: '5px 8px', cursor: 'pointer', display: 'flex', alignItems: 'center',
            color: alertCount > 0 ? 'var(--gold)' : 'var(--muted)',
            transition: 'all 0.15s'
          }}
        >
          <Bell size={14} />
