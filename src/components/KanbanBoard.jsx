...
export default function KanbanBoard({ deals, onEdit, onDelete, onUpdateStage }) {
  return (
    <div style={{
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{
                  ...
                  color: colors.text, textTransform: 'uppercase', letterSpacing: '0.06em'
                }}>
                  {stage}
                </span>
                <span style={{
              ...
              {volume > 0 && (
                <div style={{ fontSize: '11px', color: colors.text, opacity: 0.65, fontFamily: 'IBM Plex Mono, monospace', marginTop: '3px' }}>
            ...
            <div style={{ overflowY: 'auto', flex: 1, display: 'flex', flexDirection: 'column', gap: '7px', paddingBottom: '8px' }}>
              {stageDeals.length === 0 ? (
                <div style={{
                  ...
                  color: 'var(--muted)', fontSize: '12px', opacity: 0.5
                }}>
                  No deals
                </div>
              ) : (
                stageDeals.map(deal => (
                  <DealCard key={deal.id} deal={deal} onEdit={onEdit} onDelete={onDelete} onUpdateStage={onUpdateStage} />
                ))
              )}
            </div>
          </div>
