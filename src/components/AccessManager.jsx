...
import { UserPlus, Shield, ShieldOff, Trash2, Copy, Check, Users } from 'lucide-react'
...
export default function AccessManager({ session }) {
  ...
  async function fetchUsers() {
    ...
    setUsers(data || [])
    const me = (data || []).find(u => u.user_id === session.user.id)
    setIsAdmin(!data?.length || me?.role === 'admin')
    setLoading(false)
  }

  async function handleAdd() {
    if (!form.email.trim() || !form.display_name.trim() || !form.password.trim()) { setError('Email, name, and password are required.'); return }
    setAdding(true); setError(''); setSuccess('')
    const { data: signUpData, error: signUpErr } = await supabase.auth.signUp({ email: form.email.trim(), password: form.password.trim(), options: { data: { display_name: form.display_name.trim() } } })
    if (signUpErr) { setError(signUpErr.message); setAdding(false); return }
    const { error: accessErr } = await supabase.from('user_access').insert({ user_id: signUpData.user?.id || null, email: form.email.trim(), display_name: form.display_name.trim(), role: form.role, is_active: true, invited_by: session.user.id })
    if (accessErr) { setError(accessErr.message); setAdding(false); return }
    setSuccess(`Account created. Share with ${form.display_name}: ${form.email} / ${form.password}`)
    setForm({ email: '', display_name: '', role: 'member', password: '' })
    setAdding(false); fetchUsers()
  }

  async function toggleActive(user) { await supabase.from('user_access').update({ is_active: !user.is_active }).eq('id', user.id); fetchUsers() }
  async function changeRole(user, role) { await supabase.from('user_access').update({ role }).eq('id', user.id); fetchUsers() }
  async function removeUser(user) {
    if (!window.confirm(`Remove ${user.display_name || user.email}'s access? They will be immediately locked out.`)) return
    await supabase.from('user_access').delete().eq('id', user.id); fetchUsers()
  }
    ...
    <div style={{ flex: 1, overflowY: 'auto', padding: '24px' }}>
      <div style={{ maxWidth: '720px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '24px' }}>
        ...
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '10px', overflow: 'hidden' }}>
          ...
          {loading ? (
            <div style={{ padding: '24px', textAlign: 'center', color: 'var(--muted)', fontSize: '12px', fontFamily: 'IBM Plex Mono, monospace' }}>LOADING...</div>
          ) : users.length === 0 ? (
            <div style={{ padding: '24px', textAlign: 'center', color: 'var(--muted)', fontSize: '13px' }}>No team members added yet. You are the only user.<br/><span style={{ fontSize: '11px', opacity: 0.7 }}>Add yourself below to enable full access controls.</span></div>
          ) : users.map((u, i) => (
            <div key={u.id} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 18px', borderBottom: i < users.length-1 ? '1px solid var(--border)' : 'none' }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontFamily: 'Syne, sans-serif', fontWeight: 700, fontSize: '13px', color: u.is_active ? 'var(--text)' : 'var(--muted)', textDecoration: u.is_active ? 'none' : 'line-through' }}>
                  ...
                  {u.user_id === session.user.id && <span style={{ marginLeft: '6px', fontSize: '9px', background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: '3px', padding: '1px 5px', color: 'var(--muted)' }}>YOU</span>}
                </div>
          ...
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '10px', overflow: 'hidden' }}>
            ...
            <div style={{ padding: '18px', display: 'flex', flexDirection: 'column', gap: '14px' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px' }}>
                ...
                <div><label style={LS}>Temporary Password *</label><div style={{ display: 'flex', gap: '6px' }}><input value={form.password} onChange={e => setForm(f=>({...f,password:e.target.value}))} placeholder="Set a password" style={{ ...IS, flex: 1 }} /><button onClick={generatePassword} style={{ background: 'var(--surface2)', border: '1px solid var(--border)', color: 'var(--muted)', borderRadius: '6px', padding: '0 10px', cursor: 'pointer', fontSize: '10px', fontFamily: 'Syne, sans-serif', fontWeight: 600, whiteSpace: 'nowrap', flexShrink: 0 }}>AUTO</button></div></div>
              </div>
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
                <button onClick={handleAdd} disabled={adding} style={{ background: adding ? 'var(--gold-dim)' : 'var(--gold)', border: 'none', color: '#000', padding: '9px 24px', borderRadius: '6px', cursor: adding ? 'not-allowed' : 'pointer', fontFamily: 'Syne, sans-serif', fontWeight: 700, fontSize: '12px', letterSpacing: '0.04em' }}>{adding ? 'CREATING...' : 'CREATE ACCESS'}</button>
              </div>
            </div>
          </div>
        )}
        ...
        <div style={{ padding: '14px 18px', background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: '8px', fontSize: '12px', color: 'var(--muted)', lineHeight: 1.6 }}>
          <strong style={{ color: 'var(--text)', fontFamily: 'Syne, sans-serif' }}>How it works:</strong> Each team member logs in at cre-pipeline.vercel.app with their own email and password. All members share the same pipeline data. Admins can add, suspend, or remove access at any time. Suspending locks someone out immediately without deleting their account.
        </div>
      </div>
    </div>
  )
