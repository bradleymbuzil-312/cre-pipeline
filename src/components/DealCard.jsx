...
import { Edit2, Trash2, ChevronRight, Building2, Calendar, MoreHorizontal } from 'lucide-react'
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '7px' }}>
        <div style={{ fontFamily: 'Syne, sans-serif', fontWeight: 700, fontSize: '13px', color: 'var(--text)', lineHeight: 1.3, flex: 1, paddingRight: '6px' }}>
          {deal.borrower_name}
        </div>
        <div ref={menuRef} style={{ position: 'relative', flexShrink: 0 }}>
          <button onClick={() => setMenuOpen(o => !o)} style={{
            background: 'transparent', border: 'none', color: 'var(--muted)',
            cursor: 'pointer', padding: '1px 3px', display: 'flex', alignItems: 'center',
          ...
          {menuOpen && (
            <div style={{
              ...
              borderRadius: '7px', overflow: 'hidden', minWidth: '130px',
              boxShadow: '0 8px 32px rgba(0,0,0,0.5)'
            }}>
              <MenuItem icon={Edit2} label="Edit deal" onClick={() => { onEdit(deal); setMenuOpen(false) }} />
              <div style={{ height: '1px', background: 'var(--border)' }} />
              <MenuItem icon={Trash2} label="Delete" danger onClick={() => { onDelete(deal.id); setMenuOpen(false) }} />
            </div>
          )}
        </div>
      </div>
      ...
      {canAdvance && (
        <button
          onClick={() => onUpdateStage(deal.id, STAGES[currentIdx + 1])}
          style={{
            marginTop: '9px', width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '3px',
            background: 'transparent', border: '1px solid var(--border)', borderRadius: '5px',
            color: 'var(--muted)', fontSize: '10px', padding: '4px 8px', cursor: 'pointer',
            fontFamily: 'Syne, sans-serif', fontWeight: 600, letterSpacing: '0.03em',
...
function MenuItem({ icon: Icon, label, onClick, danger }) {
  return (
    <button onClick={onClick} style={{
      ...
      width: '100%', padding: '9px 13px', background: 'transparent', border: 'none',
      color: danger ? 'var(--danger)' : 'var(--text)', fontSize: '12px', cursor: 'pointer',
      fontFamily: 'DM Sans, sans-serif', fontWeight: 500, transition: 'background 0.1s',
      textAlign: 'left'
    }}
...
function Chip({ children, gold }) {
  return (
    <span style={{
      ...
      background: 'var(--surface2)', border: '1px solid var(--border)',
      color: gold ? 'var(--gold-dim)' : 'var(--muted)',
